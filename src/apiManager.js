  ...
  */
  spontaneousResponseListener(overlay) {
    ...
    window.addEventListener('message', async (event) => {
      ...
      // Kills itself if the message was not intended for Thanks to Brioche
      if (!(data && data['source'] === 'blue-marble')) {return;}

      // Kills itself if the message has no endpoint (intended for Thanks to Brioche, but not this function)
      ...
      console.log(`%cThanks to Brioche%c: Recieved message about "%s"`, 'color: cornflowerblue;', '', endpointText);

      // Each case is something that Thanks to Brioche can use from the fetch.
      ...
      switch (endpointText) {
        ...
        case 'pixel': // Request to retrieve pixel data
          ...
          // Attempt to auto-select the site's palette color based on the template and pixel
          // Only acts when live auto-color is enabled; otherwise it's a safe no-op
          try {
            const tileNums = [Number(coordsTile[0]), Number(coordsTile[1])];
            const pixelNums = [Number(coordsPixel[0]), Number(coordsPixel[1])];
            if (!Number.isNaN(tileNums[0]) && !Number.isNaN(tileNums[1]) &&
                !Number.isNaN(pixelNums[0]) && !Number.isNaN(pixelNums[1])) {
              this.templateManager?.maybeAutoSelectColor(tileNums, pixelNums);
            }
          } catch (e) { /* ignore */ }
        ...
        case 'robots': // Request to retrieve what script types are allowed
          this.disableAll = dataJSON['userscript']?.toString().toLowerCase() == 'false'; // Disables Thanks to Brioche if site owner wants userscripts disabled
          break;
      }
    });
  }
